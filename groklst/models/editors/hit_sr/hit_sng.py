...
# sys.path.insert(0, r"D:\python_projects_\SRs\Transformer-SR\HiT-SR-main")
...
class NGramContext(nn.Module):
    ...
    def seq_refl_win_pad(self, x, back=False):
        ...
        if self.padding_mode == 'zero_pad':
            return x
        if not back:
            (start_h, start_w), (end_h, end_w) = to_2tuple(
                -2 * self.ngram + 1
            ...
            (start_h, start_w), (end_h, end_w) = to_2tuple(self.ngram), to_2tuple(
                2 * self.ngram - 1
            )
            # pad upper
            x[:, :, : self.ngram - 1, :] = x[:, :, start_h:end_h, :]
...
class HiT_SNG(BaseModule, PyTorchModelHubMixin):
    """HiT-SNG network.

    title: HiT-SR: Hierarchical Transformer for Efficient Image Super-Resolution
    paper: https://eccv.ecva.net/virtual/2024/poster/2375
