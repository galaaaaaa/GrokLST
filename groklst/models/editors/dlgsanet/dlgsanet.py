...
# import sys
# sys.path.insert(
#     0, r"D:\python_projects_\SRs\Transformer-SR\DLGSANet-main\basicsr\archs"
# )
...
class IDynamicDWConvBlock(nn.Module):
    ...
    def __init__(
        ...
        if dynamic:
            self.conv = IDynamicDWConv(
                dim, kernel_size=window_size, group_channels=heads, bias=bias
            )
        else:
            self.conv = nn.Conv2d(
                dim,
    ...
    def flops(self):
        ...
        if self.dynamic:
            flops += (
                N * self.dim * self.dim / 4
                + N * self.dim * self.window_size * self.window_size
                + N
                * self.dim
                / 4
                * self.dim
...
class Attention(nn.Module):
    ...
    def __init__(
        self, dim, num_heads, bias, tlc_flag=True, tlc_kernel=48, input_resolution=None
    ):
...
class IDynamicLayerBlock(nn.Module):
    def __init__(
        ...
        self.IDynamicDWConv = IDynamicDWConvBlock(
            dim,
            window_size=window_size,
            dynamic=idynamic,
            heads=idynamic_num_heads,
        ...
        if idynamic_ffn_type == 'GDFN':
            self.IDynamic_ffn = FeedForward(
                dim,
                ffn_expansion_factor=idynamic_ffn_expansion_factor,
                bias=False,
                input_resolution=input_resolution,
            )
        elif idynamic_ffn_type == 'BaseFFN':
            self.IDynamic_ffn = BaseFeedForward(
                dim, ffn_expansion_factor=idynamic_ffn_expansion_factor, bias=True
            )
        else:
            raise NotImplementedError(
                f'Not supported FeedForward Net type{idynamic_ffn_type}'
            )
...
class DLGSANet(BaseModule):
    r"""DLGSANet

        title: DLGSANet: Lightweight Dynamic Local and Global Self-Attention Network for Image Super-Resolution
        paper: https://openaccess.thecvf.com/content/ICCV2023/html/Li_DLGSANet_Lightweight_Dynamic_Local_and_Global_Self-Attention_Networks_for_Image_ICCV_2023_paper.html
...
if __name__ == '__main__':
    ...
    print(
        f'information of input: [upscale: {upscale}] [height: {height}] [weight: {width} \n'
    )
    ...
    blocks = 4
    print(
        f'loading model TIPEIRNet with [dim: {dim}] [groups: {groups}] [blocks: {blocks} \n'
    )
    ...
    #     blocks=blocks,
    #     window_size=window_size,
    #     idynamic_num_heads=idynamic_num_heads,
